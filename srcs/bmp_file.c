...
void    bmp_header(t_all *all, int fd)
{
    ...
    size = 54 + (all->data->bits_per_pixel / 8 * all->sc->width) * all->sc->hight;
...
void    bmp_info(t_all *all, int fd)
{
    ...
    info[14] = (unsigned char)(all->data->bits_per_pixel);
    write(fd, info, 40);
    free(info);
}
...
void    bmp_file(t_all *all)
{
    ...
    y = all->sc->hight - 1;
    if ((fd = open("screen.bmp", O_CREAT | O_WRONLY | O_TRUNC, 0644)) > 1)
    {
        bmp_header(all, fd);
        bmp_info(all, fd);
        while (y >= 0)
        {
            ...
            {
                adress = (unsigned int*)(all->data->addr + all->data->line_length * y + (all->data->bits_per_pixel / 8) * x);
                write(fd, adress, 4);
                x++;
            }
            y--;
